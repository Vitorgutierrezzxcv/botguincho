...
function replaceSection(startMarker, endMarker, replacement, label) {
  ...
  if (start < 0) throw new Error(`${label}: marcador inicial nao encontrado`);
  const end = source.indexOf(endMarker, start + startMarker.length);
  if (end < 0) throw new Error(`${label}: marcador final nao encontrado`);
  source = source.slice(0, start) + replacement.trimEnd() + '\n\n' + source.slice(end);
}

function replaceOnce(oldText, newText, label) {
  if (!source.includes(oldText)) throw new Error(`${label}: bloco antigo nao encontrado`);
...
replaceSection(
  ...
  String.raw`function cleanAddressQuery(value = '') {
  return String(value)
    .replace(/\bref\.?\s*:.*$/i, '')
    .replace(/\bn[º°]\s*/gi, '')
    .replace(/[?]+$/g, '')
    .replace(/\s+/g, ' ')
    // Normaliza apenas separadores com espaco; preserva CEP, BR-381 e URLs.
    .replace(/\s+[–—-]\s+/g, ' - ')
    .trim();
}`,
  'cleanAddressQuery',
...
const coordinatesFunction = String.raw`function coordinatesFromLocation(location) {
...
replaceOnce(
  coordinatesFunction,
  coordinatesFunction + '\n\n' + String.raw`function coordinatesFromText(value = '') {
...
async function getRecentSharedLocation(groupId, stateOverride = undefined) {
  ...
  if (state?.lastSharedCoordinates && validCoordinates(state.lastSharedCoordinates.latitude, state.lastSharedCoordinates.longitude)) {
    candidates.push({
      coordinates: state.lastSharedCoordinates,
      at: new Date(state.lastSharedAt || 0).getTime(),
      source: state.lastSharedSource || 'persisted',
    });
  }
...
replaceSection(
  ...
  String.raw`const BRAZIL_STATE_BY_NAME = {
...
replaceOnce(
  ...
  String.raw`async function lookupCep(cep) {
  const digits = String(cep || '').replace(/\D/g, '');
  if (digits.length !== 8) return null;
  const response = await fetch(` + '`' + String.raw`https://viacep.com.br/ws/${digits}/json/` + '`' + String.raw`, {
...
replaceSection(
  ...
  String.raw`async function geocodeAddress(address) {
  const query = cleanAddressQuery(address);
  ...
  const save = async (found, sourceName) => {
    if (!found || !validCoordinates(found.latitude, found.longitude)) return null;
    const result = { ...found, source: sourceName, cachedAt: new Date().toISOString() };
    cache[key] = result;
    await saveGeocodeCache(cache);
    logEvent('geocode', ` + '`' + String.raw`${query} -> ${result.latitude},${result.longitude} (${sourceName})` + '`' + String.raw`);
    return result;
  };

  const directCoordinates = coordinatesFromText(query);
  if (directCoordinates) {
    return save({ ...directCoordinates, displayName: query }, 'coordinates-text');
  }

  if (extractMapsUrl(query)) {
    const mapCoordinates = await coordinatesFromMapsUrl(query);
    if (mapCoordinates) {
      return save({ ...mapCoordinates, displayName: query }, 'google-maps-url');
    }
  }

  const parts = parseBrazilAddress(query);

  // CEP informado diretamente: descobre logradouro/cidade e usa isso para desambiguar.
  if (parts.cep) {
    ...
    if (byCep) {
      const cepVariants = uniqueQueries([
        [byCep.logradouro, parts.number, byCep.bairro, ` + '`' + String.raw`${byCep.localidade} - ${byCep.uf}` + '`' + String.raw`, byCep.cep, 'Brasil'].filter(Boolean).join(', '),
        [byCep.logradouro, parts.number, ` + '`' + String.raw`${byCep.localidade} - ${byCep.uf}` + '`' + String.raw`, 'Brasil'].filter(Boolean).join(', '),
  ...
  // O texto cru entra primeiro para aceitar formatos humanos, locais e enderecos sem virgula.
  const cityState = [parts.city, parts.state].filter(Boolean).join(' - ');
  const variants = uniqueQueries([
    query,
    ` + '`' + String.raw`${query}, Brasil` + '`' + String.raw`,
  ...
  if (cep?.cep) {
    const found = await nominatimLookup({
      q: [
        cep.logradouro || parts.street,
        parts.number,
        cep.bairro,
        ` + '`' + String.raw`${cep.localidade} - ${cep.uf}` + '`' + String.raw`,
        cep.cep,
        'Brasil',
      ].filter(Boolean).join(', '),
    }).catch(() => null);
...
replaceSection(
  ...
  String.raw`async function extractMessageInput(msg) {
  ...
  if (msg.hasMedia) {
    try {
      ...
      if (media?.mimetype?.startsWith('image/') && media.data) {
        imageDataUrl = ` + '`' + String.raw`data:${media.mimetype};base64,${media.data}` + '`' + String.raw`;
      }
    } catch (error) {
...
replaceSection(
  ...
  'function extractInlineRouteTarget',
  String.raw`async function handleDispatch(msg, groupName, readableText, location) {
  ...
  try {
    ...
    logEvent('warning', 'Nao foi possivel calcular ETA do acionamento.', {
      error: String(error),
  ...
  if (eta) {
    ...
    logEvent('route', ` + '`' + String.raw`${groupName}: ETA ${eta.minutes} min${eta.distanceKm ? ` + '`' + String.raw` · ${eta.distanceKm} km` + '`' + String.raw` : ''}.` + '`' + String.raw`, { groupId: msg.from });
  }
...
replaceSection(
  ...
  String.raw`function looksLikeAddressCandidate(value = '') {
  ...
  const streetHint = /\b(rua|r\.?|avenida|av\.?|alameda|travessa|estrada|rodovia|rod\.?|br-?\d+|[a-z]{2}-?\d+|praca|largo|via|marginal|fazenda|sitio|condominio|loteamento|bairro)\b/i.test(normalized);
  const hasNumber = /\b\d{1,6}[a-z]?\b/i.test(normalized);
  const hasComma = candidate.includes(',');
  const hasUf = new RegExp(` + '`' + String.raw`\b(?:${[...BRAZIL_UFS].join('|')})\b` + '`' + String.raw`, 'i').test(candidate);
  return streetHint
    || (hasComma && (hasNumber || hasUf))
    || (hasNumber && hasUf)
    || (hasComma && candidate.length >= 12);
}
...
function extractInlineRouteTarget(text = '') {
  ...
  if (mapsUrl && asksEta(raw)) return mapsUrl;
  ...
  let candidate = raw.replace(
    /^\s*(?:(?:qual|quanto)\s+(?:a\s+|o\s+)?(?:previs[aã]o(?:\s+de\s+chegada)?|tempo(?:\s+de\s+dist[aâ]ncia)?|dist[aâ]ncia)|previs[aã]o(?:\s+de\s+chegada)?|eta|quanto\s+demora|tempo\s+(?:at[eé]|para|pra)\s+chegar)\s*[:?=\-–—]*\s*/i,
    '',
  );
  ...
  const embedded = raw.match(/\b(?:rua|r\.?|avenida|av\.?|alameda|travessa|estrada|rodovia|rod\.?|br-?\d+|[a-z]{2}-?\d+|praca|praça|largo|via|marginal|fazenda|sitio|sítio|condominio|condomínio|loteamento)\b.+$/i);
  if (embedded?.[0] && looksLikeAddressCandidate(embedded[0])) {
    return cleanAddressQuery(embedded[0]);
  }
...
async function resolveRouteQuestionTarget(groupId, readableText) {
  ...
  return {
    ...
    targetCoordinates: inlineAddress
      ? null
      : (sharedIsNewer ? shared.coordinates : state?.originCoordinates || null),
    source: inlineAddress
      ? 'inline-address'
      : (sharedIsNewer ? shared.source : 'dispatch-state'),
  };
}`,
  'resolucao generica de destino',
);
...
replaceOnce(
  ...
  `    if (location && !text) {\n      logEvent('system', \`\${groupName}: localizacao do WhatsApp armazenada; aguardando pergunta ou acionamento.\`, {\n        groupId: msg.from,\n      });\n      return;\n    }`,
  'armazenamento de localizacao',
);
