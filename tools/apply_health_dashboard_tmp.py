...
for name in ['index.html','public/index.html']:
    ...
    if 'id="healthOverall"' not in s:
        if marker not in s: raise SystemExit(f'health html anchor missing {name}')
...
for name in ['sw.js','public/sw.js']:
    p=Path(name); s=p.read_text(); s=s.replace("bot-guincho-pwa-v1","bot-guincho-pwa-v2"); p.write_text(s)
