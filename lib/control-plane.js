...
export async function signupWithPassword(email, password) {
  ...
  if (normalizedEmail !== BOOTSTRAP_MASTER_EMAIL) {
    const error = new Error('email_not_authorized_for_first_access');
    error.status = 403;
    throw error;
  }
...
export async function requireSession(req) {
  ...
  // Idempotente: ao primeiro acesso, qualquer convite pendente para o e-mail autenticado
  // vira company_members. Nenhum tenant é liberado apenas pelo companyId da URL.
  try {
