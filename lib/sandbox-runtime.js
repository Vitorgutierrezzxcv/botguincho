...
const SANDBOX_NAME = 'botguincho-wa-vercel-v10';
...
function bootstrap(credential = '') {
  ...
  return [
    ...
    'export OPENAI_MODEL=openai/gpt-5.4-mini',
    ...
    `for i in $(seq 1 90); do if ${localHealth()}; then echo READY; rm -rf "$LOCK"; exit 0; fi; sleep 1; done`,
    'echo "Worker não respondeu"',
    'tail -160 /vercel/sandbox/worker.log 2>/dev/null || true',
    'rm -rf "$LOCK"',
    'exit 1',
  ].join('\n');
}
...
async function kick(sandbox, credential = '') {
  ...
  const encoded = Buffer.from(bootstrap(credential), 'utf8').toString('base64');
  const launcher = [
    `printf '%s' ${quote(encoded)} | base64 -d > /vercel/sandbox/bootstrap-run.sh`,
    'chmod 700 /vercel/sandbox/bootstrap-run.sh',
    'nohup bash /vercel/sandbox/bootstrap-run.sh > /vercel/sandbox/bootstrap-launch.log 2>&1 < /dev/null &',
    'echo $! > /vercel/sandbox/bootstrap-launch.pid',
    'echo STARTED',
  ].join(' && ');

  try {
    await run(sandbox, launcher, { timeoutMs: 6000 });
  } catch {}
}
...
export async function getWorkerStatus(credential = '') {
  ...
  try {
    ...
    return {
      ...
      groupsSelected: 0,
      infrastructure: { status: 'starting', sandbox: SANDBOX_NAME, message: 'Preparando WhatsApp e IA em segundo plano.' },
    };
  } catch (error) {
    return {
      clientId: 'cliente-teste',
...
export async function sandboxDiagnostics(credential = '') {
  ...
  try {
    ...
    const result = await run(sandbox, [
      ...
      'echo "--- PROCESSES ---"; ps -ef | grep -E "bootstrap-run|vercel-whatsapp-worker" | grep -v grep || true',
      'echo "--- CHROMIUM ---"; ls -l /tmp/chromium /tmp/al2023/lib/libnspr4.so /tmp/al2023/lib/libnss3.so 2>/dev/null || true',
      'echo "--- LAUNCH ---"; tail -80 /vercel/sandbox/bootstrap-launch.log 2>/dev/null || true',
      'echo "--- BOOTSTRAP ---"; tail -180 /vercel/sandbox/bootstrap.log 2>/dev/null || true',
      'echo "--- WORKER ---"; tail -180 /vercel/sandbox/worker.log 2>/dev/null || true',
    ].join('; '), { timeoutMs: 6000 });
    return {
      ok: true,
      ready,
      ...
      workerStatus,
      logs: (await result.stdout()).slice(-20000),
