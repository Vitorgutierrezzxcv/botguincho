...
function bootstrapScript() {
  ...
  return [
    ...
    '    echo "Bootstrap já está em execução."',
    '    exit 0',
    '  fi',
    '  echo "Removendo trava antiga."',
    '  rm -rf "$LOCK"',
    '  mkdir "$LOCK"',
    'fi',
    'echo $$ > "$LOCK/pid"',
    ...
    'echo "Preparando diretórios e configurações."',
    ...
    '  OLD_PID=$(cat /vercel/sandbox/botguincho-worker.pid 2>/dev/null || true)',
    '  if [ -n "${OLD_PID:-}" ] && kill -0 "$OLD_PID" 2>/dev/null; then',
    '    echo "Encerrando worker antigo PID $OLD_PID."',
...
export async function getWorkerStatus() {
  try {
    ...
    return {
      ...
      ai: { configured: aiConfigured(), enabled: true, model: 'openai/gpt-5-mini' },
      groupsSelected: 0,
      infrastructure: {
...
function initializingPayload(internalPath) {
  ...
  if (internalPath === '/api/settings') return { ...defaultSettings(), apiKeyConfigured: aiConfigured(), initializing: true };
...
export async function proxyWorker(req, res, internalPath) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      return res.status(200).json({ ...initializingPayload(internalPath), warning: error instanceof Error ? error.message : String(error) });
    }
    return res.status(503).json({ error: 'worker_unavailable', message: error instanceof Error ? error.message : String(error) });
  }
}
...
export async function sandboxDiagnostics() {
  try {
    ...
    let workerLogs = '';
    try {
      const result = await exec(sandbox, 'printf "%s\\n" "--- BOOTSTRAP ---"; tail -120 /vercel/sandbox/botguincho-bootstrap.log 2>/dev/null || true; printf "%s\\n" "--- WORKER ---"; tail -120 /vercel/sandbox/botguincho-worker.log 2>/dev/null || true', { timeoutMs: 5000 });
      bootstrapLogs = await result.stdout();
    } catch {}
    try {
      const result = await exec(sandbox, 'ps aux | grep -E "admin-platform|npm install|git clone" | grep -v grep || true', { timeoutMs: 5000 });
      workerLogs = await result.stdout();
    } catch {}
