...
export function externalWorkerConfigured() {
  // Acionador.ai opera exclusivamente na infraestrutura da Vercel.
  // Variáveis legadas da antiga VPS são ignoradas para impedir fallback externo.
  return false;
}
...
export async function maintainWorker(credential = '', tenant = DEFAULT_CLIENT_ID) {
  const id = sanitizeTenant(tenant);
  rememberCredential(credential, id);
  if (externalWorkerConfigured() && id === DEFAULT_CLIENT_ID) {
...
export async function proxyWorker(req, res, internalPath) {
  ...
  try {
    ...
    if (externalWorkerConfigured() && tenant === DEFAULT_CLIENT_ID) {
