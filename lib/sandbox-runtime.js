...
export async function refreshWorkerRuntime(credential = '', tenant = DEFAULT_CLIENT_ID) {
  const id = sanitizeTenant(tenant);
  rememberCredential(credential, id);
  const sandbox = await ensureWorkerSandbox(id);

  await run(sandbox, {
    cmd: 'bash',
    args: ['-lc', [
      "pkill -f '[n]ode tools/vercel-whatsapp-worker.mjs' >/dev/null 2>&1 || true",
  ...
  await installDependencies(sandbox);
  const patchStatus = await applyWwebjsPatch(sandbox);
  await configureSessionWindow(sandbox);
  await startWorkerDetached(sandbox, id);
  await new Promise((resolve) => setTimeout(resolve, 5000));

  const status = await readWorkerStatus(sandbox, id);
  const logResult = await run(sandbox, {
    cmd: 'bash',
    args: ['-lc', [
      "echo '--- HEAD ---'",
      "git rev-parse HEAD",
      "echo '--- VPS FILES ---'",
      "ls Dockerfile.vps compose.vps.yml .env.vps.example ops/Caddyfile.vps 2>/dev/null || true",
      "echo '--- PROCESS ---'",
      "ps aux | grep -E 'vercel-whatsapp-worker|chromium|chrome' | grep -v grep || true",
      "echo '--- LOG ---'",
      "tail -120 /vercel/sandbox/worker.log 2>/dev/null || true"
    ].join('; ')],
  }, 8000);
  let logs = '';
  try { logs = await logResult.stdout(); } catch {}
...
export async function runWorkerValidation(tenant = DEFAULT_CLIENT_ID) {
  const id = sanitizeTenant(tenant);
  const sandbox = await ensureWorkerSandbox(id);
  const results = [];

  async function execute(name, command, timeoutMs = 120000) {
    const result = await run(sandbox, command, timeoutMs);
    let stdout = '';
    let stderr = '';
  ...
  await execute('test-type-deps', { cmd: 'npm', args: ['install', '--no-save', '@types/express@5.0.3', '--no-audit', '--no-fund'] }, 60000);
  await execute('typescript-build', { cmd: 'npm', args: ['run', 'build'] });
  await execute('operational-tests', { cmd: 'npm', args: ['run', 'test:operational'] });
  await execute('cancellation-tests', { cmd: 'npm', args: ['run', 'test:cancellation'] });
  await execute('business-tests', { cmd: 'npm', args: ['run', 'test:business'] });
  await execute('full-business-flow', { cmd: 'npm', args: ['run', 'test:full'] });
  await execute('worker-syntax', { cmd: 'node', args: ['--check', 'tools/vercel-whatsapp-worker.mjs'] }, 30000);

  const smokeScript = String.raw`
const checks = [];
async function check(name, path, init) {
  try {
    const r = await fetch('http://127.0.0.1:3001' + path, init);
    const text = await r.text();
    checks.push({ name, ok: r.ok, status: r.status, sample: text.slice(0, 600) });
  } catch (error) {
...
await check('management', '/api/management');
await check('billing', '/api/billing');
await check('learning-summary', '/api/learning/summary');
await check('groups', '/api/groups');
await check('health', '/api/health');
await check('route-test', '/api/route-test', {
  method: 'POST',
  headers: { 'content-type': 'application/json' },
  body: JSON.stringify({
    from: 'Rua André Luiz Pereira, 263, Betim - MG',
    to: 'Praça Sete de Setembro, Belo Horizonte - MG'
  })
});
await check('ai-ignore-test', '/api/ai-test', {
  method: 'POST',
  headers: { 'content-type': 'application/json' },
  body: JSON.stringify({ text: 'bom dia' })
});
process.stdout.write(JSON.stringify(checks));
`;
  await execute('http-smoke', { cmd: 'node', args: ['--input-type=module', '-e', smokeScript] }, 60000);
  await execute('restore-production-deps', { cmd: 'npm', args: ['prune', '--omit=dev', '--no-audit', '--no-fund'] }, 60000);
...
export async function trackerPairingCode(tenant = DEFAULT_CLIENT_ID, { rotate = false } = {}) {
  const id = sanitizeTenant(tenant);
  const sandbox = await ensureWorkerSandbox(id);
  const pairFile = `${DATA_DIR}/${id}/gconnect-pair-code.txt`;
  const readingFile = `${DATA_DIR}/${id}/gconnect-reading.json`;
  const script = rotate
    ? `const fs=require('fs');const crypto=require('crypto');const f=${JSON.stringify(pairFile)};const r=${JSON.stringify(readingFile)};fs.mkdirSync(require('path').dirname(f),{recursive:true});const c=crypto.randomBytes(6).toString('base64url').replace(/[^A-Z0-9]/gi,'').toUpperCase().slice(0,8).padEnd(8,'9');fs.writeFileSync(f,c,{mode:0o600});try{fs.rmSync(r,{force:true})}catch{};process.stdout.write(c)`
    : `const fs=require('fs');const crypto=require('crypto');const f=${JSON.stringify(pairFile)};fs.mkdirSync(require('path').dirname(f),{recursive:true});let c='';try{c=fs.readFileSync(f,'utf8').trim().toUpperCase()}catch{};if(!/^[A-Z0-9]{8}$/.test(c)){c=crypto.randomBytes(6).toString('base64url').replace(/[^A-Z0-9]/gi,'').toUpperCase().slice(0,8).padEnd(8,'7');fs.writeFileSync(f,c,{mode:0o600})};process.stdout.write(c)`;
  const result = await run(sandbox, { cmd: 'node', args: ['-e', script] }, 8000);
  let code = '';
  try { code = (await result.stdout()).trim().toUpperCase(); } catch {}
  if (result.exitCode !== 0 || !/^[A-Z0-9]{8}$/.test(code)) throw new Error('tracker_pairing_failed');
  return { ok: true, companyId: id, pairCode: code, rotated: rotate };
}
