...
const SANDBOX_NAME = 'botguincho-wa-vercel-v7';
...
function bootstrap() {
  ...
  const aiEndpoint = quote(`${publicUrl()}/api/ai/respond`);
  return [
    'set -eu',
    ...
    `export BOTGUINCHO_AI_ENDPOINT=${aiEndpoint}`,
    ...
    `for i in $(seq 1 45); do if ${localHealth()}; then echo READY; exit 0; fi; sleep 1; done`,
...
async function kick(sandbox) {
  if (await isReady(sandbox)) return;
  try {
    await run(sandbox, bootstrap(), { detached: true, timeoutMs: 8000 });
  } catch {}
}
...
export async function getWorkerStatus() {
  try {
    ...
    return {
      ...
      groupsSelected: 0,
      infrastructure: { status: 'starting', sandbox: SANDBOX_NAME, message: 'Preparando WhatsApp.' },
    };
  } catch (error) {
    return {
      clientId: 'cliente-teste',
...
function placeholder(path) {
  ...
  if (path === '/api/settings') return { ...defaultSettings(), apiKeyConfigured: true, initializing: true };
...
export async function sandboxDiagnostics() {
  try {
    ...
    const result = await run(sandbox, [
      ...
      'echo "--- PROCESSES ---"; ps aux | grep -E "vercel-whatsapp-worker|npm install|chromium" | grep -v grep || true',
    ].join('; '), { timeoutMs: 6000 });
    return { ok: true, ready, sandbox: SANDBOX_NAME, aiGatewayConfigured: true, workerStatus, logs: (await result.stdout()).slice(-18000), remainingIntegration: 'gconnect' };
  } catch (error) {
