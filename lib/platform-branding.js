...
const SUPABASE_ANON_KEY = process.env.SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXAiLCJyZWYiOiJwcmlibmR5d2d1YWNla2FmaHV5ayIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNzg2ODk2NDk0LCJleHAiOjIxMDI0NzI0OTR9.xHIYFkWzymWQl4iJYBOSGc5SVB0ce44Eh72m5c0C7bM';
...
export async function getPlatformBranding({ includeAssets = false } = {}) {
  const fields = includeAssets
    ? 'id,platform_name,short_name,tagline,pwa_description,primary_color,logo_data_url,app_icon_data_url,favicon_data_url,updated_at'
...
export function publicBrandingPayload(row = {}) {
  ...
  return {
    ...
    has_app_icon: Boolean(merged.app_icon_data_url),
    has_favicon: Boolean(merged.favicon_data_url),
    logo_url: `/api/branding/asset?kind=logo&v=${stamp}`,
    app_icon_url: `/api/branding/asset?kind=app_icon&v=${stamp}`,
    favicon_url: `/api/branding/asset?kind=favicon&v=${stamp}`,
    manifest_url: `/api/branding/manifest?v=${stamp}`,
  };
}

export function assetDataUrl(row = {}, kind = '') {
  if (kind === 'logo') return row.logo_data_url || '';
  if (kind === 'favicon') return row.favicon_data_url || row.app_icon_data_url || '';
  if (kind === 'app_icon') return row.app_icon_data_url || '';
  return '';
}
