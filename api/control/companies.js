...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') return res.status(200).json({ companies: await listCompanies() });
    if (req.method === 'POST') return res.status(201).json({ company: await createCompany(req.body || {}) });
    const id = String(req.body?.id || '').trim();
    if (!id) return res.status(400).json({ error: 'id_required' });
    return res.status(200).json({ company: await updateCompany(id, req.body?.patch || {}) });
  } catch (error) {
